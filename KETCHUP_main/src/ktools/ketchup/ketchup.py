...
collects basic functions that for working with KETCHUP specific models
...
def ketchup_model_options() -> dict:
    """
    Creates options dictionary with default values for KETCHUP.
    ...
    model_options: dict[str, str | bool | int | None] = {
        ...
        'directory_model': f"{os.getcwd()}/data/",  # location of model and mechanism files
        ...
        'solver_opt_fn': None,  # file for solver options
    }
...
def ketchup_generate_model(ketchup_options: dict) -> pyomo.core.base.PyomoModel.ConcreteModel:
    ...
    if ketchup_options['input_format'].lower() in ("kfit", "k-fit"):
        ...
        raise ValueError(f"Invalid input style format. Currently only supports K-FIT style.")
