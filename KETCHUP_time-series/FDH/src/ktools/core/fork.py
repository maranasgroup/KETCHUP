...
def create_initial_model(m_model,mech_df,data,seedvalue=None,time=None,tde = 0, k_thres=10**5,distribution='uniform',mech_type='elemental'):
    ...
    if mech_type == 'elemental':
        ...
        model.kcat = Var(bounds = (eps,k_thres) )
        #define Km 
        model.reactants = Set(initialize = ["nad","hco2"])
        model.Km_r   =  Var(model.reactants, bounds = (eps,k_thres) )
        #define Ki
        model.inhibitors = Set(initialize = ['nad','nadh'])
        model.Ki     =  Var(model.inhibitors, bounds=(eps,k_thres) )
        #lower bound for nadh decomposition https://www.sciencedirect.com/science/article/pii/S0003267016300290#sec4
        model.K_decomp = Var(bounds=(0,k_thres))
        #nadh_decomp = {'a1':0.000177191325680275, 'a2':0.00015002655124346585 ,
        #       'b1':6.048310705184825e-06 ,'c1':0.0012860661297633034}
        ...
        randomize_Ks(['nad','nadh'], model.Ki, e_scale, distribution)
...
def create_dMB(b):
    ...
    ##FOR FDH specifically
    b.model().rxn_enz_sum['FDH'][1] = data[key]['enzyme']  
    ...
    model.c[0,'hco2'] = data[key]['formate']
    model.c[0,'nad'].fixed = True
    model.c[0,'hco2'].fixed = True
    model.c[0,'nadh'].fixed = True
    
    model.c[0,'nadh'] = data[key]['nadh']
    model.c[0,'co2'] = data[key]['co2']
    model.c[0,'co2'].fixed = True
...
def create_dKM( rxn_enz_sum, m_model, mech_df, mech_type, time, data, k_thres=10**10 ):
    ...
    model.rate = Var(model.TIME, model.REACTIONS, bounds=(eps,10*4))
    ...
    def hco2_conc_bounds(m,t):
        return (m.data['formate']*1 + m.data['co2']) - m.c[t,'hco2'] >= 0
    def co2_conc_bounds(m,t):
        return (m.data['formate']*1 + m.data['co2']) - m.c[t,'co2'] >= 0
    ...
    model.hco2_conc = Constraint(model.TIME, rule=hco2_conc_bounds)    
    model.nadh_conc = Constraint(model.TIME, rule=nadh_conc_bounds)
    model.co2_conc = Constraint(model.TIME, rule=co2_conc_bounds)  
    
    model.enz_sum = Constraint(model.TIME, model.REACTIONS, rule=d_enz_sum)  
    
    #for dynamic data
    #colloc = TransformationFactory('dae.collocation')  
    #colloc.apply_to(model,ncp = 3,nfe = 2*(len(time)), wrt=model.TIME, scheme='LAGRANGE-RADAU') 
...
#08/05/2024 CUSTOM RATE LAWS
#need to define automatic way to transcribe written equation into symbolic form
def fdh_rate(b,t):
    ...
    den = b.model().Ki['nad'] * b.model().Km_r['hco2'] + \
          b.model().Km_r['hco2'] * b.c[t,'nad'] + \
          b.model().Km_r['nad'] * b.c[t,'hco2'] + \
          b.c[t,'nad'] * b.c[t,'hco2'] + \
          (b.model().Ki['nad']*b.model().Km_r['hco2'])/(b.model().Ki['nadh'])*b.c[t,'nadh'] + \
